...
"""
function issplit(A::AbsAlgAss{fmpq})
  i = schur_index(A, inf)
  if i == 2
...
function issplit(A::AbsAlgAss{nf_elem})
  K = base_ring(A)
...
function issplit(A::AbsAlgAss, P::InfPlc)
  if iscomplex(P)
    return true
  end
  return schur_index(A, P) == 1
...
# of base_ring(A)
function iseichler(A::AbsAlgAss{nf_elem})
  @assert issimple(A)
  @assert iscentral(A)
...
function iseichler(A::AbsAlgAss{fmpq})
  @assert issimple(A)
  @assert iscentral(A)
  if dim(A) != 4
    return true
